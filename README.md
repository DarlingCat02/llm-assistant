...
Обычные текстовые модели (qwen3.5, llama, etc.) не поддерживают vision через API.
...
1. **Проверьте модель в LM Studio** — должна быть VL-модель (qwen2.5-vl, llava, bakllava), НЕ текстовая (qwen3.5, llama)
2. **Проверьте `.env`** — `LLM_MODEL` должен указывать на VL-модель
3. **Перезапустите бэкенд** после смены модели в LM Studio
3. **Контекст переполнен** — увеличьте `LLM_NUM_CTX` в `.env` (8192 или 16384)
...
3. **400 Bad Request при скриншоте** — модель не VL или превышен контекст (увеличьте `LLM_NUM_CTX`)
